...
function attestChip(attested: number, total: number): { label: string; textColor: string; bg: string; border: string } {
  ...
  if (pct >= 1)    return { label:`${total}/${total} attested`, textColor:"#059669", bg:"#ecfdf5", border:"#a7f3d0" };
  if (pct >= 0.5)  return { label:`${attested}/${total} attested`, textColor:"#b45309", bg:"#fffbeb", border:"#fde68a" };
  return           { label:`${attested}/${total} attested`, textColor:"#be123c", bg:"#fff1f2", border:"#fecdd3" };
}
...
export default function ScansPage() {
  ...
  function ScanRow({ s }: { s: ScanSummary }) {
    ...
    const riskTotal   = s.high_crit_count ?? s.file_count;
    const chip        = attestChip(Math.min(attested, riskTotal), riskTotal);
