...
const SIGNAL_DESC: Record<string, string> = {
  ...
  "bola-missing-ownership-check": "A Spring @PathVariable/@RequestParam-sourced identifier reaches a repository/map lookup with no @PreAuthorize/@Secured/@RolesAllowed and no identity comparison anywhere in the method",
  "plaintext-password-storage": "Password assigned directly from request input with no hashing function anywhere on the line",
  "debug-mode-enabled": "Framework debug mode is explicitly enabled — exposes stack traces, source code, and (Werkzeug) an interactive RCE console",
...
const SEV_SIGNAL: Record<string, string> = {
  ...
  "file-inclusion":"#f97316","weak-signing-secret":"#7c3aed","graphql-introspection-enabled":"#f59e0b","bola-identity-mismatch":"#f97316","bola-missing-ownership-check":"#f97316","plaintext-password-storage":"#f97316","debug-mode-enabled":"#f97316",
};
    ...
    <div className="border-t border-gray-100">
      ...
      <div className="flex items-center justify-between px-5 py-3"
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => { navigator.clipboard.writeText(content).catch(() => {}); setCopied(true); setTimeout(() => setCopied(false), 2000); }}
...
const REMEDIATION: Record<VType, string[]> = {
  ...
  unattested_medium:   [
    "Review the MEDIUM-risk file in the PR detail view",
    "Confirm the AI-generated sections are intentional and understood",
    "Attest the file before the next quarterly review deadline",
  ],
  ai_threshold:        [
...
export default function ViolationsPage() {
  ...
  const violations = useMemo<(Violation & { status: VStatus; assignee?: string })[]>(() => {
    ...
    return [...active, ...resolvedExtras].map(v => ({
      ...v,
      status:   (statuses[v.id] as VStatus) ?? "open",
  ...
  function setStatus(id: string, status: VStatus) {
    ...
    if (status === "in_review") {
      ...
      setFilterStat("in_review");
      info("Review started", "Code panel opened below — review and mark resolved when done.");
    }
    else if (status === "resolved") {
      setReviewingId(null);
      setFilterStat("open");
      success("Violation resolved", "Status updated and saved.");
    }
  ...
  // Scope card counts to current severity + repo filters so they match the visible list
  const cardBase = violations.filter(v => {
    if (filterSev  !== "all" && v.severity !== filterSev)  return false;
    if (filterRepo !== "all" && v.repo     !== filterRepo) return false;
    return true;
  });
  const open     = cardBase.filter(v => v.status === "open").length;
  const critical = cardBase.filter(v => v.severity === "CRITICAL" && v.status !== "resolved").length;
  const slaOver  = cardBase.filter(v => v.sla_deadline && new Date(v.sla_deadline) < new Date() && v.status !== "resolved").length;
  const inReview = cardBase.filter(v => v.status === "in_review").length;
          ...
          {violations.length === 0 && !loading && !loadError ? (
            ...
            return (
              <div key={v.id}
                      ...
                      <>
                        ...
                        {v.scan_id && v.file ? (
                          <button onClick={() => setStatus(v.id, "in_review")}
                            className="text-[11px] font-medium text-amber-700 bg-amber-50 border border-amber-200 px-3 py-1.5 rounded-lg hover:bg-amber-100 transition-colors whitespace-nowrap flex items-center gap-1">
                            <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                          ...
                          <Link href={`/repo/${v.repo}`}
                            className="text-[11px] font-medium text-indigo-700 bg-indigo-50 border border-indigo-200 px-3 py-1.5 rounded-lg hover:bg-indigo-100 transition-colors whitespace-nowrap flex items-center gap-1">
                            <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                      ...
                      <>
                        ...
                        {v.scan_id && v.file && (
                          <button onClick={() => setReviewingId(reviewingId === `hide::${v.id}` ? null : `hide::${v.id}`)}
                            ...
                            {reviewingId === `hide::${v.id}` ? "View Code" : "Hide Code"}
                          </button>
                        )}
                      </>
                    )}
                ...
                {/* Inline code review panel — shown when in_review and not explicitly hidden */}
                {v.status === "in_review" && reviewingId !== `hide::${v.id}` && v.scan_id && v.file && (
                  <InlineCodeReview
                    scanId={v.scan_id}
                    filePath={v.file}
